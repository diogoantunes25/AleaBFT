...
public class Dealer {
    ...
    // TODO: Merge Dealer.generateShares and Dealer.generateVerifiers
    // and generate them simultaneously
    public static BigInteger generateVerifiers(final BigInteger n, final KeyShare[] secrets) {
    ...
    public static void main(final String[] args) {
        ...
        final int[] S1 = { 0, 1, 2 };
        ShoupSigShare[] sigs1 = new ShoupSigShare[K];
        for (int i = 0; i < S1.length; i++)
            sigs1[i] = keys[S1[i]].sign(b);
        System.out.println(ShoupSigShare.combine(b, sigs1, K, L, gk.getModulus(), gk.getExponent()));

        final int[] S2 = { 2, 3, 4 };
        ShoupSigShare[] sigs2 = new ShoupSigShare[K];
        for (int i = 0; i < S2.length; i++)
            sigs2[i] = keys[S2[i]].sign(b);
        System.out.println(ShoupSigShare.combine(b, sigs2, K, L, gk.getModulus(), gk.getExponent()));

        System.out.println(
                ShoupSigShare.combine(b, sigs1, K, L, gk.getModulus(), gk.getExponent())
                        .equals(ShoupSigShare.combine(b, sigs2, K, L, gk.getModulus(), gk.getExponent())));
    }
}
