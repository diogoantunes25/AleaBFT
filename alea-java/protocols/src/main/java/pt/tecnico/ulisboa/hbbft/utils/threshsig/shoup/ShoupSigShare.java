 ...
 */
public class ShoupSigShare extends SigShare implements Serializable {

    // Constants and variables
    //............................................................................
    private final static boolean CHECKVERIFIER = false; // FIXME true;
    ...
    // Constructors
    //............................................................................
    public ShoupSigShare(final int id, final BigInteger sig, final Verifier sigVerifier) {
        this.id = id;
        this.sig = sig;
        this.sigVerifier = sigVerifier;
    }

    public ShoupSigShare(final int id, final byte[] sig) {
        this.id = id;
    ...
    // Static methods
    //............................................................................
    public static BigInteger combine(final byte[] data, final ShoupSigShare[] sigs, final int k, final int l,
        ...
        final BigInteger delta = ShoupSigShare.factorial(l);
            ...
            try {
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    if (!result.equals(ver.getC())) {
                        throw new ThresholdSigException("Share verifier is not OK");
                    }
                }
        ...
        for (int i = 0; i < k; i++) {
            w = w.multiply(sigs[i].getSig().modPow(ShoupSigShare.lambda(sigs[i].getId(), sigs, delta), n));
        }
        return w.mod(n);
    }

    public static boolean verify(final byte[] data, final ShoupSigShare[] sigs, final int k, final int l,
                                 final BigInteger n, final BigInteger e) throws ThresholdSigException {
        // Sanity Check - make sure there are at least k unique sigs out of l
        // possible
     ...
     */
    private static BigInteger lambda(final int ik, final ShoupSigShare[] S,
                                     final BigInteger delta) {
        // lambda(id,l) = PI {id!=j, 0<j<=l} (i-j')/(id-j')
        BigInteger value = delta;

        for (final ShoupSigShare element : S) {
            if (element.getId() != ik) {
    ...
    protected byte getNum() {
        return SigShareType.SHOUP.getFirst();
    }
    ...
    public static ShoupSigShare fromBytes(byte[] payload) {
        ByteArrayInputStream bis = new ByteArrayInputStream(payload);
        ObjectInput ois = null;
        ShoupSigShare sigShare;
        try {
            ois = new ObjectInputStream(bis);
            sigShare = (ShoupSigShare) ois.readObject();
        } catch (IOException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
