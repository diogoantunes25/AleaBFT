...
def main():
    ...
    number = "" if f < n/2 else "1"
    with open(os.path.join(f"keys-{n}", f"sPK{number}.key"), "wb") as fh:
        pickle.dump(keys[0], fh)

    for i in range(n):
        with open(os.path.join(f"keys-{n}", f"sSK{number}-{i}.key"), "wb") as fh:
            pickle.dump(keys[1][i], fh)
